...
while (!$pl -> EOF) {
	...
	if( is_file( "avatars/{$pl->fields['avatar']}" ) ) {
  	...
  	if($pl->fields['jail']=='Y')
		...
		$tmpp['data'] = $pl->fields;
		$tmpp['title'] = $overcaption;
		$tmpp['img'] = $imglink;
		$tmpp['body'] = $overlib;
		$tmpp['link'] = "<a href=\"view.php?view={$pl->fields['id']}\">{$pl->fields['user']}</a> ";
		if($pl->fields['tribe']>0) {
			if(file_exists('images/tribes/mini/'.$pl->fields['tribe'].'.jpg'))
				$tmpp['data']['tribeimg'] = $pl->fields['tribe'].'.jpg';
		}
		if( !isset( $tmpp['data']['tribeimg'] ) )
			$tmpp['data']['tribeimg'] = '';
...
$smarty -> assign (array ("Players" => $nump, "Online" => $numo, "List" => $arrplayers, "Duration" => $duration, "Show" => $show, "Sqltime" => $sqltime, "PHPtime" => $phptime, "Numquery" => $numquery));
