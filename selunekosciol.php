<?php
$title = "Kosciol Selune";
require_once("includes/head.php");
$db -> Execute("UPDATE players SET page='Kosciol Selune' WHERE id=".$player -> id);
if (isset ($_GET['action']) && $_GET['action'] == 'chat') {
     if (isset ($_POST['msg'])) {
        ...
        $czat = $db -> Execute("SELECT gracz FROM selunekosciolconfig");
        while (!$czat -> EOF) {
            if ($player -> id == $czat -> fields['gracz']) {
                error ("Nie mozesz pisac wiadomosci w Kosciele Selune");
            }
            $czat -> MoveNext();
        }
        $czat -> Close();
        ...
        $db -> Execute("INSERT INTO selunekosciol (user, chat, senderid,ownerid) VALUES('".$starter."', '".$message."',".$player -> id.",".$owner.")");
        for ($i = 0; $i < 8; $i++) {
            $empty = 0;
            if ($test == "Niewolniku ".$arritems[$i]." dla "){
                $id = substr($_POST['msg'],20);
                if ($id == 'wszystkich!') {
                           $db -> Execute("INSERT INTO selunekosciol (user, chat) VALUES('<i>Niewolnik</i>', 'Uwaga! ".$player -> user." stawia wszystkim ".$arritems[$i]."!')");
                       }
                if (ereg("^[1-9][0-9]*$", $id)) {
                    $db -> Execute("INSERT INTO selunekosciol (user, chat) VALUES('<i>Niewolnik</i>', 'Prosze ".$user -> fields['user']." oto ".$arritems[$i]." od ".$player -> user."')");
                }
                break;
        ...
        if ($_POST['msg'] == 'Niewolniku jakies wiesci?') {
            ...
            if ($number > 0) {
                ...
                $db -> Execute("INSERT INTO selunekosciol (user, chat) VALUES('<i>Niewolnik</i>', 'Niestety nic ciekawego sie nie wydarzylo')");
            }
        }
        if ($_POST['msg'] == 'Ochroniarzu jakis problem?') {
            ...
            if ($number > 0) {
                $roll = rand (1,$number);
                $db -> Execute("INSERT INTO selunekosciol (user, chat) VALUES('<i>Ochroniaz</i>', 'Nie niema zadnego, tak patrze na twoja bron i sie boje')");
            }
        }
        if ($empty == 1) {
            ...
            $roll = rand (0, 4);
            $db -> Execute("INSERT INTO selunekosciol (user, chat) VALUES('<i>Niewolnik</i>', '".$arrtext[$roll]."')");
        }
    }
}
...
$smarty -> display ('selunekosciol.tpl');
