...
$title = "Swiatynia Wladcy Bitew";
require_once("includes/head.php");

checklocation($_SERVER['SCRIPT_NAME']);

if( $player -> id != 267 ) {
	error( "Koscioly jak narazie zamkniete !" );
}

if ($player -> deity != 'Tempus') {
	error ("Odejdz innowierco !");
}
...
// funkcja odpowiedzialna za modlitwe do danego boga
	...
	if ($arrprayer[$rzut1] == 20) {
		...
		if (isset ($_GET['temp']) && $_GET['temp'] == 'sluzba') {
			if( strlen( $player->deity ) <=0 ){
				var_dump( empty( $player->deity ) );
				error ("Nie mozesz modlic sie w swiatyni poniewaz jestes ateista! `{$player -> deity}`");
			}
			if (isset($_GET['dalej'])) {
				if ($_POST['rep'] <= 0) {
		...
		if (isset ($_GET['temp']) && $_GET['temp'] == 'modlitwa') {
			...
			if (isset($_GET['modl'])) {
				...
				if (empty ($player -> deity)) {
					error ("Nie mozesz sie modlic poniewaz jestes ateista! `{$player -> deity}`");
				}
				$pw1 = 0;
				$rzut = rand(1,100);
				$pw = $player -> pw;
				...
				if ($rzut <= $pw1) {
					...
					if ($player -> deity == 'Bane') {
						$arrtemp = array(0,1,2,3,4,6,9,11,12,14,15,16);
						$amount = 12;
					}
...
$smarty -> display('tempustemple.tpl');
