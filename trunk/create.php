...
if( strlen( $player->oczy ) && strlen( $player->deity ) ) {
	error( "Twoja postaæ ju¿ jest utworzona ! ", 'error', 'stats.php' );
}

$deityTab = array( 'azuth', 'bane', 'chauntea', 'cyric', 'eilistraee','gond', 'helm', 'ilmater', 'kelemvor', 'kossuth', 'lathander', 'lolth', 'malar', 'mask', 'mielikki', 'oghma', 'selune', 'shar', 'sune', 'talos', 'tempus', 'torm', 'tymora', 'tyr', 'umberlee', 'waukeen' );
...
switch( $step ) {
	case 'race' :
		...
		break;
	case 'gender' :
		$rData = SqlExec( "SELECT name FROM races WHERE code_name='{$crt['race']}'" );
		$rData = array_shift( $rData->GetArray() );
		//qDebug( $rData );
		$smarty->assign( "RaceName", $rData['name'] );
		break;
	case 'class' :
	...
	case 'done' :
		...
		if( !$cDone ) {
			...
			foreach( $atrArr as $el ) {
				...
				$summ['atr'][$el] = $player->$el + $bon * ( $rName->fields[$el] + $clName->fields[$el] ) + $famTab[$crt['familiar']][$el];
			}
			//qDebug( $summ );
