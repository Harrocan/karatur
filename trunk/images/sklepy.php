	...
	if ($_GET['limit'] < $przed) {
		...
		while (!$pm -> EOF) {
			...
			if ($player -> id == $pm -> fields['owner']) {
				$arraction[$i][0] = "<td><a href=imarket.php?wyc=".$pm -> fields['id']."><font color='darkblue'>";
				$arraction[$i][1]= "</font></a></td>";
			} else {
...
elseif (isset ($_GET['view']) && $_GET['view'] == 'del') {
	...
	if (isset($_GET['step']) && $_GET['step'] == 'buy') {
		...
		SqlExec("INSERT INTO log (owner, log, czas) VALUES(".$item['owner'].",'<b><a href=view.php?view=".$player -> id.">".$player -> user."</a></b> zaakceptowal twoja oferte za <b>".$_POST['amount']." sztuki {$olditem['prefix']} ".$olditem['name']."</b>. Dostales <b>".$price."</b> sztuk zlota do banku.','".$newdate."')");
