<?php
$title = "Ksciol Lathandera";
require_once("includes/head.php");
$db -> Execute("UPDATE players SET page='Kosciol Lathandera' WHERE id=".$player -> id);
if (isset ($_GET['action']) && $_GET['action'] == 'chat') {
     if (isset ($_POST['msg'])) {
        ...
        $czat = $db -> Execute("SELECT gracz FROM lathanderkosciolconfig");
        while (!$czat -> EOF) {
            if ($player -> id == $czat -> fields['gracz']) {
                error ("Nie mozesz pisac wiadomosci w Kosciele Lathandera");
            }
            $czat -> MoveNext();
        }
        $czat -> Close();
        ...
        $arritems = array('modlmy sie', 'wino', 'cialo', 'slonce', 'poranek', 'dobro', 'prawo');
        ...
        $db -> Execute("INSERT INTO latanderkosciol (user, chat, senderid,ownerid) VALUES('".$starter."', '".$message."',".$player -> id.",".$owner.")");
        for ($i = 0; $i < 8; $i++) {
            $empty = 0;
            if ($test == "Wierzacy".$arritems[$i]." dla "){
                $id = substr($_POST['msg'],20);
                if ($id == 'wszystkich!') {
                           $db -> Execute("INSERT INSERT lathanderkosciol (user, chat) VALUES('<i>Chor</i>', '".$arritems[$i]."!')");
                       }
                if (ereg("^[1-9][0-9]*$", $id)) {
                    $db -> Execute("INSERT INTO lathanderkosciol (user, chat) VALUES('<i>Niewolnik</i>', 'Prosze ".$user -> fields['user']." oto ".$arritems[$i]." od ".$player -> user."')");
                }
                break;
            } elseif ($checkbot == "Wierzacy") {
                $empty = 1;
            }
        }
        if ($_POST['msg'] == 'Niewolniku jakies wiesci?') {
            ...
            if ($number > 0) {
                ...
                $db -> Execute("INSERT INTO lathanderkosciol (user, chat) VALUES('<i>Niewolnik</i>', '".$text."')");
            } else {
                $db -> Execute("INSERT INTO lathanderkosciol (user, chat) VALUES('<i>Niewolnik</i>', 'Niestety nic ciekawego sie nie wydarzylo')");
            }
        }
        if ($_POST['msg'] == 'Ochroniarzu jakis problem?') {
            ...
            if ($number > 0) {
                $roll = rand (1,$number);
                $db -> Execute("INSERT INTO lathanderkosciol (user, chat) VALUES('<i>Ochroniaz</i>', 'Nie niema zadnego, tak patrze na twoja bron i sie boje')");
            }
        }
        if ($empty == 1) {
            ...
            $roll = rand (0, 4);
            $db -> Execute("INSERT INTO lathanderkosciol (user, chat) VALUES('<i>Karczmarz</i>', '".$arrtext[$roll]."')");
        }
    }
}

$query = $db -> Execute("SELECT id FROM lathanderkosciol");
$numchat = $query -> RecordCount();
$query -> Close();
$smarty -> assign ("Number", $numchat);
$smarty -> display ('lathanderkosciol.tpl');
